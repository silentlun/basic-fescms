...
class UploadAction extends Action{
    ...
    public function ActUpload(){
        ...
        if ($upload != null){
            ...
            $ext = strtolower($upload->getExtension());//扩展名
            //判断文件类型'png', 'jpg', 'jpeg', 'gif', 'mp3', 'mp4', 'm3u8'
            if( !in_array($ext, explode('|', Yii::$app->config->site_upload_allowext)) ){
                return [
                "state" => '格式不允许',
                ];
            }
            //判断文件大小
            if ($ext == 'pdf') {
                if ($upload->size > Yii::$app->config->site_upload_pdfmaxsize*1024){
                    return [
                        "state" => '文件大小超出限制',
                    ];
                }
